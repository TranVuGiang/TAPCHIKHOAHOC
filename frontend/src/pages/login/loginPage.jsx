...
import loginGIF from '@/assets/ChristmarsLogingif.gif';
...
function LoginPage() {
    ...
    const colors = ['red', 'green', 'brown', 'ameral']; // Danh sách các màu

    useEffect(() => {
        const interval = setInterval(() => {
            setColorIndex((prevIndex) => (prevIndex + 1) % colors.length); // Đổi màu
        }, 2000); // 2 giây
    ...
    return (
        <section className="min-h-screen bg-red-950 flex items-center justify-center font-montserrat">
            <div className="bg-white flex justify-center rounded-2xl shadow-lg max-w-3xl w-full p-5">
                <div className="md:w-1/2 px-10 pt-7">
                    <h2 className="font-bold text-2xl text-red-800 text-center">Đăng nhập</h2>
                    <p
                        style={{ color: colors[colorIndex], transition: 'color 0.5s ease' }}
            ...
            ${errors.password ? 'border-red-500' : ''}`}
                        ...
                        <button
                            type="submit"
                            className="bg-red-700 rounded-lg text-white py-2 
                                hover:bg-emerald-800 transition duration-300 disabled:opacity-50"
                            disabled={isLoading}
                        >
                    ...
                    >
                        <span className="cursor-pointer text-red-800 hover:text-emerald-500 hover:underline">
                            Quên mật khẩu?
                        </span>
                    </p>
                    <GoogleLoginButtons />
                    <div className="mt-3 text-xs  flex justify-between items-center">
                        <p className="text-red-800 hover:text-emerald-500">Không có tài khoản...</p>
                        <button
                            onClick={() => (window.location.href = '/home/register_user')}
                            className="py-2 px-5 hover:text-emerald-500 text-red-800 bg-white border rounded-xl 
                ...
                <div className="md:block hidden w-1/2">
                    <img src={loginGIF} alt="LoginG" className="rounded-2xl" />
                </div>
            </div>
        </section>
    );
