...
const UserPermissionManagement = () => {
    ...
    const handlePermissionUpdate = async (userId, status) => {
        ...
        const current = JSON.parse(localStorage.getItem('currentUser'));
        try {
            await authService.updatequyenAuthor({
                token: current.token,
                taikhoanId: userId,
                status: status,
            });
            ...
            <div className="py-8">
                <div className="border rounded-lg shadow-lg">
                    <div className="px-4 py-5 sm:px-6 bg-gray-50 border-b">
                        <h3 className="text-lg leading-6 font-semibold text-gray-900">Quản Lý Cấp Quyền Người Dùng</h3>
                    </div>
                    {error === null ? (
                        <div className="text-center text-gray-500 p-4">Không có yêu cầu cấp quyền nào</div>
                    ) : (
